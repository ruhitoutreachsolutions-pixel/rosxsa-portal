    ...
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-0 sm:pl-10">
        <div className="w-screen max-w-md bg-brand-navy border-l border-brand-midnight shadow-2xl flex flex-col">
          ...
          {/* Desktop Notifications Push Settings Strip */}
          <div className="px-6 py-2.5 bg-brand-black/60 border-b border-brand-midnight flex items-center justify-between gap-2">
            <div className="flex items-center gap-1.5 text-xs">
              <Bell className="w-3.5 h-3.5 text-brand-cyan" />
              <span className="text-brand-gray text-[11px]">Desktop Push Alerts:</span>
              {desktopPermission === 'granted' ? (
                <span className="text-brand-green font-semibold text-[11px] flex items-center gap-1">
                  <Check className="w-3 h-3" /> Active
                </span>
              ) : desktopPermission === 'denied' ? (
                <span className="text-red-400 font-semibold text-[11px]">Blocked in browser</span>
              ) : (
                <span className="text-yellow-400 font-semibold text-[11px]">Disabled</span>
              )}
            </div>
            ...
            {desktopPermission !== 'granted' && (
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-brand-cyan/20 hover:bg-brand-cyan/30 text-brand-cyan border border-brand-cyan/40 text-[11px] font-bold transition-all shadow-cyan-glow"
              >
