      ...
      <div className="bg-brand-navy border border-brand-midnight rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl animate-scale-up">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          {(stage === 'invoice_sent' || stage === 'payment_pending') && (
            <div className="p-4 rounded-xl bg-brand-navy border border-brand-orange/30 space-y-3.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono text-brand-gray">
                  Overdue Alert: <strong className="text-brand-orange font-bold">{followUpDays} Days</strong>
                </span>
              </div>
              ...
              {/* Overdue Notification Time Selector */}
              <div className="pt-2 border-t border-brand-midnight/60 space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <label className="text-brand-gray font-medium flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5 text-brand-orange" />
                    <span>Overdue Notification Time / Follow-up Threshold</span>
                  </label>
                  <span className="text-[11px] font-mono text-brand-cyan">
                    Triggers desktop alert after {followUpDays} days
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                  {[3, 5, 7, 10, 14].map((days) => (
                    <button
                      ...
                      className={`py-2 px-3 rounded-xl text-xs font-mono font-bold transition-all border flex items-center justify-center gap-1.5 ${
                        followUpDays === days
                          ? 'bg-brand-orange/20 text-brand-orange border-brand-orange shadow-orange-glow scale-[1.02]'
                          : 'bg-brand-black text-brand-gray border-white/10 hover:text-white hover:border-white/20'
                      }`}
                    >
